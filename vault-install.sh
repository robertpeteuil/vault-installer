...
# VAULT INSTALLER - Automated HashiCorp Vault Installation
#   Apache 2 License - Copyright (c) 2019  Robert Peteuil  @RobertPeteuil
#
#     Automatically Download, Extract and Install
#        Latest or Specific Version of vault
#
#   from: https://github.com/robertpeteuil/vault-installer
...
usage() {
  [[ "$1" ]] && echo -e "Download and Install Vault - Latest Version unless '-i' specified\n"
...
getLatest() {
  ...
  case "${nettool}" in
    ...
    wgetjq)
      LATEST_ARR=($(wget -q -O- https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.vault.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    curljq)
      LATEST_ARR=($(curl -s https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.vault.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    # parse version from github API - use Tags for vault as release not populated
    wget)
      LATEST_ARR=($(wget -q -O- https://api.github.com/repos/hashicorp/vault/tags 2>/dev/null | grep name | awk '{print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2 | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    curl)
      LATEST_ARR=($(curl -s https://api.github.com/repos/hashicorp/vault/tags 2>/dev/null | grep name | awk '{print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2 | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
  esac
...
[[ $PROC =~ arm ]] && PROC="arm"  # vault downloads use "arm" not full arm type

# CREATE FILENAME AND URL FROM GATHERED PARAMETERS
FILENAME="vault_${VERSION}_${OS}_${PROC}.zip"
LINK="https://releases.hashicorp.com/vault/${VERSION}/${FILENAME}"
SHALINK="https://releases.hashicorp.com/vault/${VERSION}/vault_${VERSION}_SHA256SUMS"
...
if [[ ! "$cwdInstall" ]]; then
  ...
  ${CMDPREFIX} cp -f vault "$BINDIR" || exit 1

  # CLEANUP AND EXIT
  cd "${TMPDIR}" || exit 1
  rm -rf "${UTILTMPDIR}"
  [[ ! "$STREAMLINED" ]] && echo
  echo "vault Version ${VERSION} installed to ${BINDIR}"
else
  echo "Vault Version ${VERSION} downloaded"
fi
